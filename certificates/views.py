...
from .permissions import IsOwnerOrIssuerOrAdmin, IsOrganizationOrAdmin
...
class CertificateDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    serializer_class = CertificateSerializer
    permission_classes = [IsOwnerOrIssuerOrAdmin]
...
class CertificateTemplateListCreateView(generics.ListCreateAPIView):
    ...
    permission_classes = [IsOrganizationOrAdmin]
...
def bulk_create_certificates(request):
    """Bulk create certificates from Excel/CSV data"""
    if request.user.role not in ['organization', 'admin']:
        return Response(
            {'error': 'Faqat tashkilotlar sertifikat yarata oladi'},
